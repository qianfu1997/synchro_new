...
this updated one will produce 81(num_classes) x 9(inst_embeds)
and select the corresponding inst_embeds according to labels.
If you need to predcit a precise mask, that you need 
to crop mask firstly and then extract features of them.

Another question is that, the anchor can have misaligning 
"""
import numpy as np
import torch
...
class YolactBboxHeadPlus(AnchorHead):
    """
    Anchor-based head.
    Args:
        num_classes,
    ...
    def __init__(self,
        ...
        super(YolactBboxHeadPlus, self).__init__(
    ...
    def _init_layers(self):
        ...
        for i in range(self.num_emb_convs):
            chn = self.in_channels if i == 0 else self.feat_channels
            self.emb_convs.append(ConvModule(
                chn,
                self.feat_channels,
                3,
                stride=1,
        ...
        self.yolact_emb = nn.Conv2d(
            # for each anchor type and each classes
            self.feat_channels, self.num_anchors * self.num_classes * self.inst_embeds, 3, padding=1)
    ...
    def forward_single(self, x):
        ...
        for emb_conv in self.emb_convs:
            emb_feat = emb_conv(emb_feat)

        cls_scores = self.yolact_cls(cls_feat)
        bbox_pred = self.yolact_reg(reg_feat)
        # [N, num_anchors * num_classes * in_embeds, H, W]
        bbox_embeds = self.yolact_emb(emb_feat)
        bbox_embeds = self.tanh(bbox_embeds)

        return cls_scores, bbox_pred, bbox_embeds
